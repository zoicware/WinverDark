...
{
	button = CreateWindow(L"button", L"", WS_CLIPSIBLINGS | WS_CHILD | WS_TABSTOP | WS_VISIBLE, 0, 0, 0, 0, hWnd, (HMENU)199, hInst, NULL);
	HRGN hRgn = CreateRoundRectRgn(0, 0, 71, 24, 8, 8);
	//HRGN hRgn = CreateRoundRectRgn(3, 2, 69, 22, 8, 8); //no border
	SetWindowRgn(button, hRgn, TRUE);
...
BOOL CustomDrawButton(LPARAM lParam, HWND hWnd)
{
	if (((LPNMHDR)lParam)->idFrom == 199)
	{
		...
		Graphics graphicshdc(item->hdc);
		graphicshdc.SetSmoothingMode(SmoothingModeAntiAlias);

		SolidBrush lightmodetext(Color(255, 0, 0, 0));
		SolidBrush darkmodetext(Color(255, 255, 255, 255));
		...
		RectF rectF(item->rc.left, item->rc.top, item->rc.right - item->rc.left, item->rc.bottom - item->rc.top);
		SolidBrush* textBrush = DarkThemeEnabled ? &darkmodetext : &lightmodetext;

		if (item->uItemState & CDIS_SELECTED)
		{
			graphicshdc.DrawString(L"OK", -1, &font, rectF, &stringFormat, textBrush);
		}
